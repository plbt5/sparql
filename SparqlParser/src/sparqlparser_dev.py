...
class Terminal(SPARQLNode):
    def assignFields(self):
        self.elements = OrderedDict([('token', ''.join(self.tokens))])
    def render(self):
        return self.elements['token']
            
class NonTerminal(SPARQLNode):

    def assignFields(self):
        keyList = [t.getName() for t in self.tokens]
        print('***keylist***:', keyList)
        valueList = [getattr(self.tokens, k) for k in keyList]
        self.elements = OrderedDict(zip(keyList, valueList))
    def render(self, sep=' '):
...
def inspect(s, depth=0):
    indent = '  '
    if isinstance(s, str):
        print(indent*depth + '<str>', s)
    elif isinstance(s, SPARQLNode):
        for k in s.elements.keys():
            print(indent*depth + k + ': ')
            inspect(s.elements[k], depth + 1)
        return
    else:
...
def printOrderedDict(od, depth=0):
    indent = '  '
    for k in od.keys():
        print(indent*depth + k + ': ', end='')
        if isinstance(od[k], str):
            print(od[k])
        elif isinstance(od[k], SPARQLNode):
            assert isinstance(od[k].elements, OrderedDict)
            print()
            printOrderedDict(od[k].elements, depth + 1)
        else:
            assert isinstance(od[k], ParseResults)
            print()
            printParseResults(od[k], depth + 1)

            
def printParseResults(pr, depth=0):
    indent = '  ' * depth
    for t in pr:
        if isinstance(t, str):
            print(indent + '<str>', t)
            ...
            assert isinstance(t.elements, OrderedDict)
            printOrderedDict(t.elements, depth + 1)
        else:
            assert isinstance(t, ParseResults)
            printParseResults(t, depth + 1)
...
RDFLiteral_p = Group(String_p)('lexical_form') + Optional( Group ( ( LANGTAG_p ^ (Suppress('^^') + iri_p ) ) ) ('type_info') ) 
