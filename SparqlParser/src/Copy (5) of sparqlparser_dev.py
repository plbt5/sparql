...
class NonTerminal(SPARQLNode):
    def assignFields(self):
        self.elements = namedtuple(self.__class__.__name__ + '_nt', [k for k in self.tokens.asDict().keys()])._make([self.tokens.asDict()[k] for k in self.tokens.asDict().keys()])
    def render(self, sep=' '):
...
#             print(v)        
            reslist.append(v.render())
        return sep.join(reversed(reslist))  
# 
...
BooleanLiteral_e = r'({})|({})'.format( 'true', 'false')
BooleanLiteral_p = Regex(BooleanLiteral_e)
class BooleanLiteral(Terminal): pass
BooleanLiteral_p.setParseAction(BooleanLiteral)
...
class RDFLiteral(NonTerminal):
    ...
    def render(self):
        if len(self.elements._fields) > 1 and isinstance(self.elements.type_info[0][0], iri):
...
class Expression(NonTerminal):
    def assignFields(self):
        self.elements_prefix = 'Expression_'
        NonTerminal.assignFields(self)
Expression_p.setParseAction(Expression)
 
# [71]    ArgList   ::=   NIL | '(' 'DISTINCT'? Expression ( ',' Expression )* ')' 
ArgList_p =   NIL_p ^ ( LPAR_p + Optional(DISTINCT_p) + delimitedList(Expression_p) + RPAR_p )
...
iriOrFunction_p = iri_p + Optional(ArgList_p)
