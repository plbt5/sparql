...
from pyparsing import ParseFatalException, Regex, Literal, Combine, Optional,\
    Forward, CaselessKeyword, delimitedList, Suppress
import sys
...
#         return 'Class:' + self.__class__.__name__ + ', dict:' + str(self.__dict__)
        return 'Class:' + self.__class__.__name__ + ', fields: ' + str(self.fields)
    def __repr__(self):
        return 'Class:' + self.__class__.__name__ + ', dict:' + str(self.__dict__)
        
class Terminal(SPARQLNode): 
    def assignFields(self):
        self.fields['chars'] = ''.join([t if isinstance(t, str) else t.asString() for t in self.tokens])
    def asString(self):
        return self.fields['chars']
        
class NonTerminal(SPARQLNode):
    def assignFields(self):
        pass
    def asString(self):
        return ' '.join([t if isinstance(t, str) else t.asString() for t in self.tokens])
...
LPAR_p, RPAR_p, SEMICOL_p, COMMA_p = map(Suppress, '();,')
...
RDFLiteral_p = String_p + Optional( ( LANGTAG_p ^ ('^^' + iri_p) ) )
class RDFLiteral(NonTerminal):
    def assignFields(self):
        self.result = self.tokens
    def asString(self):
        return ''.join([t if isinstance(t, str) else t.asString() for t in self.result])
