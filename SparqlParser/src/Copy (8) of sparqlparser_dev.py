...
def analyse(s, depth=0):
    indent = '  '
    if isinstance(s, str):
        print(indent*depth + '<str>', s)
    elif isinstance(s, SPARQLNode):
        for k in s.elements._fields:
            print(indent*depth + k + ': ')
            analyse(getattr(s.elements, k), depth + 1)
        return
    else:
        assert isinstance(s, ParseResults)
        for r in s:
            print(indent*depth + '[' + r.__class__.__name__ + ']')
            analyse(r, depth+1)    
            
class Terminal(SPARQLNode):
    def assignFields(self):
        self.elements = namedtuple(self.__class__.__name__ + '_nt', ['token'])(''.join(self.tokens))
    def render(self):
        return getattr(self.elements, 'token')
            
class NonTerminal(SPARQLNode):
    def assignFields(self):
        # pyparsing's asDict() returns the items in random order, not necessarily in the order in which they appear in in the pattern. Therefore the keys are prefixed by 'Xn_' in the pattern and sorted before use.
        keys = sorted(self.tokens.asDict().keys())
        self.elements = namedtuple(self.__class__.__name__ + '_nt', keys)._make([self.tokens.asDict()[k] for k in keys])
        
#     def render(self, sep=' '):
# #         print('keys:', self.elements._fields)
# #         print('items:')
#         reslist = []
#         for k in self.elements._fields:
#             v = getattr(self.elements, k)
#             while isinstance(v, ParseResults):
# #                 assert(len(v) == 1)
#                 v = v[0]
# #             assert isinstance(v, SPARQLNode)
# #             print(v)        
#             reslist.append(v if isinstance(v, str) else v.render())
#         return sep.join(reslist) 

        
    def unpackParseResults(self, pr):
        if isinstance(pr, str):
            return [pr]
        if isinstance(pr, SPARQLNode):
            return [pr]
#         assert isinstance(pr, ParseResults), type(pr)
        return reduce(lambda x, y: x + y, [self.unpackParseResults(p) for p in pr])
    
    def render(self, sep=' '):
        reslist = []
        assert len(self.elements._fields) > 0
        for k in self.elements._fields:
#             print('k =', k)
            v = getattr(self.elements, k)
            if isinstance(v, str):
                reslist.append(v)
                continue
            if isinstance(v, SPARQLNode):
                reslist.extend(v.render())
                continue
            assert isinstance(v, ParseResults), type(v)
#                 print('len(v) =', len(v), ', v =', v)
#                 v = v[0]
            reslist.append([v1 if isinstance(v1, str) else v1.render() for v1 in self.unpackParseResults(v)])
        return sep.join(reduce(lambda x, y: x + y, reslist))  
...
RDFLiteral_p = Group(String_p)('E1_lexical_form') + Optional( Group ( ( LANGTAG_p ^ (Suppress('^^') + iri_p ) ) ) )('E2_type_info') 
...
class ExpressionList(NonTerminal):
    def __init__(self, tokens):
        self.elements = NonTerminal(tokens).elements
    def render(self):
...
ArgList_p =   Group(NIL_p)('E1_nil') | Group( LPAR_p + Optional(DISTINCT_p)('F1') + expressionList_p('F2') + RPAR_p )('E2_expression_list')
class ArgList(NonTerminal): pass
ArgList_p.setParseAction(ArgList)


# [128]   iriOrFunction     ::=   iri ArgList? 
iriOrFunction_p = iri_p('E1_iri') + Optional(Group(ArgList_p))('E2_ArgList')
