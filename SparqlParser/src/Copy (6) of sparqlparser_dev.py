...
class NonTerminal(SPARQLNode):
    def assignFields(self):
        self.elements = namedtuple(self.__class__.__name__ + '_nt', [k for k in sorted(self.tokens.asDict().keys())])._make([self.tokens.asDict()[k] for k in sorted(self.tokens.asDict().keys())])
    def render(self, sep=' '):
...
RDFLiteral_p = Group(String_p)('X1_lexical_form') + Optional( Group ( ( LANGTAG_p ^ (Suppress('^^') + iri_p ) ) ) )('X2_type_info') 
class RDFLiteral(NonTerminal):
    def asString(self):
        return self.joinChars()
    def render(self):
        if len(self.elements._fields) > 1 and isinstance(self.elements.X2_type_info[0][0], iri):
